...
let rec optimize env e = 
  ...
  | ID id -> 
    ...
    | Some x -> optimize env x
    | None -> e)
  | Fun (var, exptype, expr) -> Fun (var, exptype, optimize env expr)
  | Not (e1) -> 
    (match e1 with
    | ID id -> 
      (match lookup env id with
  ...
  | If (guard, tbranch, fbranch) -> 
    ...
    let opt_tb = optimize env tbranch in
    let opt_fb = optimize env fbranch in 
    (match opt_guard with
    | Bool true -> opt_tb
    | Bool false -> opt_fb
    | ID x -> If (opt_guard, opt_tb, opt_fb)
    | _ -> If (opt_guard, opt_tb, opt_fb) ) 
